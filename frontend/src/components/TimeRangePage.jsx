...
function TimeRangePage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const params = { startDate: toLocalIsoNoZ(start), endDate: toLocalIsoNoZ(end) };
      ...
      const filtered = raw
        ...
        .sort((a, b) => {
          ...
          if (ad === bd) return (a.name ?? a.stationName ?? a.locatn ?? '').localeCompare(b.name ?? b.stationName ?? b.locatn ?? '');
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'25px', height:'100%', padding:'25px', background:'#f0f2f5' }}>
      {/* 상단 필터 */}
      <div style={{ background:'#fff', padding:'50px 40px', borderRadius:10, boxShadow:'0 2px 5px rgba(0,0,0,.1)', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:25 }}>
        <div style={{ display:'flex', alignItems:'center', gap:25, flexWrap:'wrap', fontSize:22 }}>
          <strong>기간설정</strong>
          <select style={{ padding:14, borderRadius:5, border:'1px solid #ccc', fontSize:22 }} defaultValue="daily">
          ...
          <input
            ...
            style={{ padding:14, borderRadius:5, border:'1px solid #ccc', fontSize:22 }}
          />
        ...
        <div style={{ display:'flex', alignItems:'center', gap:15 }}>
          <button
            ...
            disabled={loading}
            style={{ padding:'14px 28px', border:'none', borderRadius:5, background:'#6c757d', color:'#fff', cursor:'pointer', fontSize:22 }}
          >
            {loading ? '검색 중…' : '검색'}
          </button>
        </div>
      ...
      {/* 본문: 표 + 다운로드 버튼 */}
      <div style={{ background:'#fff', borderRadius:10, padding:30, boxShadow:'0 2px 5px rgba(0,0,0,.1)', flexGrow:1, display:'flex', flexDirection:'column' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
          <h3 style={{ margin:0, fontSize:30 }}>수질 예측 데이터</h3>
          <div>
            <button title="CSV 다운로드 (준비중)" style={{ padding:'12px 18px', borderRadius:5, border:'1px solid #ccc', background:'transparent', cursor:'pointer', fontSize:20, marginRight:10 }}>
              <span style={{ marginRight:8 }}>⬇️</span> CSV
            </button>
            <button title="EXCEL 다운로드 (준비중)" style={{ padding:'12px 18px', borderRadius:5, border:'1px solid #ccc', background:'transparent', cursor:'pointer', fontSize:20 }}>
        ...
        {error && (
          <div style={{ color:'crimson', marginBottom:10, textAlign:'center', padding:20, fontSize:22 }}>
            {error}
          </div>
        )}
        {!loading && !error && rows.length === 0 && (
          <div style={{ color:'#666', textAlign:'center', padding:'50px 20px', fontSize:22 }}>
            검색 결과가 없습니다.
          </div>
        )}

        {rows.length > 0 && (
          <div style={{ overflow:'auto', border:'1px solid #e5e7eb', borderRadius:8, flexGrow:1 }}>
